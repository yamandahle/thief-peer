...
match against yamanagh-cop: sub-game 1 audited clean on both sides, but
every record in sub-games 2-6 came back "tampered" on the Cop's own
side -- its `wait_for_turn(1, ...)` for sub-game 2 was instantly
returning sub-game 1's own leftover step-1 message, still sitting in
`_turns` from before, rather than actually waiting for the new one.
`series_runner.play_series` must call `reset_turns()` at the start of
every sub-game to close this -- this repo has the identical class, so
the same fix applies here even though this side's own run happened not
to expose it that day.
"""
