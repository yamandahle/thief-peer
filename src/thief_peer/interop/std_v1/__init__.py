...
Deliberately reuses this repo's own `McpTransport` (infra/mcp_client.py)
unchanged for every outbound call here -- one persistent connection held
for the whole series, narrowed retry to provably pre-transmission
failures only. std_v1's own tools are all fire-and-forget enqueue
handlers (Section 7: "validate lightly, enqueue, return {"ok": true}
immediately"), so a blind retry after any failure would in principle be
safe for this protocol specifically -- but there is no reason to widen
the transport's own safety margin just because one protocol happens not
to need it; the narrower policy costs nothing here and stays the single,
uniform rule for every protocol this repo speaks.
"""
