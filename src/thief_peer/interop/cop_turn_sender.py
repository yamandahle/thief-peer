...
def cop_send_commit(transport, h_commit: str) -> dict:
    return transport.call("receive_commit", {"h_commit": h_commit})


def cop_send_reveal(transport, move: str, hint_text: str) -> dict:
    return transport.call(
        "receive_reveal", {"move": {"type": "move", "direction": move}, "hint_text": hint_text}
    )
...
def cop_send_barrier_declaration(transport, row: int, col: int) -> dict:
    return transport.call("receive_barrier_declaration", {"col": col, "row": row})
